...
passport.use(
  new LocalStrategy(async (username, password, done) => {
    try {
      ...
      const mongoUser = await MongoClient.db.collection('users').findOne({ username: username });
      if (mongoUser) {
      ...
      const pgUserResult = await Pool.query('SELECT * FROM users WHERE username = $1', [username]);
...
passport.deserializeUser(async (id, done) => {
  try {
    // Check MongoDB for user
    const mongoUser = await MongoClient.db.collection('users').findOne({ _id: id });
    if (mongoUser) return done(null, mongoUser);

    // Check PostgreSQL for user
    const pgUserResult = await Pool.query('SELECT * FROM users WHERE id = $1', [id]);
    const pgUser = pgUserResult.rows[0];
